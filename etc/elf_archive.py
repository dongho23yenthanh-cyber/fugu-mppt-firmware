...
Layout (under elf-archive/, gitignored):
  blobs/<sha8>.elf.zst   one zstd -19 ELF per unique build (~9.7 MB vs ~35 MB)
  blobs/<sha8>.json      build meta {version, date, time, idf, sha}
  index.jsonl            one line per flash event

CLI:
  elf_archive.py archive <device> [--method ota|serial] [--at ISO] [--elf P] [--bin P]
...
ARCHIVE_DIR = os.path.join(REPO_ROOT, 'elf-archive')
BLOBS_DIR = os.path.join(ARCHIVE_DIR, 'blobs')
INDEX = os.path.join(ARCHIVE_DIR, 'index.jsonl')
DEFAULT_ELF = os.path.join(REPO_ROOT, 'build', 'fugu-firmware.elf')
DEFAULT_BIN = os.path.join(REPO_ROOT, 'build', 'fugu-firmware.bin')
APP_DESC_MAGIC = 0xABCD5432
ZSTD_LEVEL = '19'
KEEP_BUILDS = 30  # retention cap: prune all but the 30 most-recently-flashed builds
...
def archive(device, method='ota', elf=DEFAULT_ELF, bin=DEFAULT_BIN, when=None,
            version=None, background=True):
    ...
    if os.path.exists(blob):
        ...
        if background:
            # detached (own session) so it outlives ota.py/flash.sh; argv-only
            # (no shell) so paths can't be misread as shell metacharacters.
            subprocess.Popen([sys.executable, __file__, '_compress', elf, tmp, blob],
                             start_new_session=True,
                             stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            print(f'archiving build {sha8} -> {os.path.relpath(blob, REPO_ROOT)} '
                  f'(compressing in background)')
        else:
            _compress(elf, tmp, blob)
            print(f'archived build {sha8} ({os.path.getsize(blob) / 1e6:.1f} MB) '
                  f'-> {os.path.relpath(blob, REPO_ROOT)}')
...
def extract(sha, dest=None):
    ...
    if dest is None:
        fd, dest = tempfile.mkstemp(prefix=f'fugu-{sha[:8]}-', suffix='.elf')
        os.close(fd)
...
def main():
    ...
    a.add_argument('--elf', default=DEFAULT_ELF)
